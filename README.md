# 👨‍💻 Imran Ahmad — Senior DevOps & AI Platform Engineer (In Training)

Hi! I’m Imran, a passionate **DevOps Engineer with 5 years of experience** transitioning aggressively into an **AI-driven Platform Engineering** role in 2027. I specialize in building highly resilient infrastructure, optimizing CI/CD pipelines, and integrating AI Agents into operational workflows. 

I believe great engineering goes beyond uptime—it's about **Zero-Trust security, hyper-efficiency, and autonomous systems**.

---

## 🚀 The DevOps 2027 Command Center

I am currently executing a rigorous 5-month career accelerator program, documenting every single lab, architecture decision, and system debugging scenario.

🌐 **Check out my Live Progress Dashboard:** [imranshs08.github.io/job/](https://imranshs08.github.io/job/)  
🐙 **Explore the Source Repository:** [imranshs08/job](https://github.com/imranshs08/job)

### 🎯 Certifications In Progress
- **CKA (Certified Kubernetes Administrator):** Target Date — Jan 1, 2027
- **AZ-104 (Azure Administrator):** Target Date — Jan 15, 2027

---

## 🛠️ Core Technology Stack

### ☁️ Cloud & Orchestration
[![AWS](https://img.shields.io/badge/AWS-%23FF9900?style=for-the-badge&logo=amazon-aws&logoColor=white)](https://aws.amazon.com/) 
[![Azure](https://img.shields.io/badge/azure-%230072C6.svg?style=for-the-badge&logo=microsoftazure&logoColor=white)](https://azure.microsoft.com/en-us/)
[![Kubernetes](https://img.shields.io/badge/Kubernetes-%23326ce5?style=for-the-badge&logo=kubernetes&logoColor=white)](https://kubernetes.io/) 
[![Docker](https://img.shields.io/badge/Docker-%230db7ed?style=for-the-badge&logo=docker&logoColor=white)](https://www.docker.com/)

### 🔁 Automation & IaC
[![Terraform](https://img.shields.io/badge/Terraform-%235835CC?style=for-the-badge&logo=terraform&logoColor=white)](https://www.terraform.io/)
[![Ansible](https://img.shields.io/badge/Ansible-%231A1918?style=for-the-badge&logo=ansible&logoColor=white)](https://www.ansible.com/)
[![Jenkins](https://img.shields.io/badge/Jenkins-%23D24939?style=for-the-badge&logo=jenkins&logoColor=white)](https://www.jenkins.io/)
[![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?style=for-the-badge&logo=github-actions&logoColor=white)](https://github.com/features/actions)

### 🐧 Languages & Scripting
[![Python](https://img.shields.io/badge/Python-%233776AB?style=for-the-badge&logo=python&logoColor=white)](https://www.python.org/)
[![Shell](https://img.shields.io/badge/Shell_Scripting-%23121011?style=for-the-badge&logo=gnu-bash&logoColor=white)](https://www.gnu.org/software/bash/)
[![Linux](https://img.shields.io/badge/Linux-%23FCC624?style=for-the-badge&logo=linux&logoColor=black)](https://www.linux.org/)

---

## 🧠 Current Upskill Tracks

- **Cloud Platform:** Migrating legacy enterprise workloads to AKS/EKS with Zero-Trust principles.
- **AI Integration:** Leveraging Claude/GPT/Local LLMs for AIOps, automated log triage, and defensive script hardening.
- **System Stability:** Deep diving into Linux `strace`, Systemd cgroup isolation, and Datadog/Prometheus alert fatigue mitigation.

---

## 📈 GitHub Stats

![Imran's GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=imranshs08&theme=dark)
![Imran's GitHub Stats](https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=imranshs08&theme=dark)

---

## 🤝 Let's Connect

📫 **Email:** [imranshs08@gmail.com](mailto:imranshs08@gmail.com)  
💼 **LinkedIn:** [Ahmad Imran](https://www.linkedin.com/in/ahmadimranjmi/)  
🌍 **Location:** India

> *"Automation isn’t just writing scripts — it’s designing trust into your systems."* 🚀
